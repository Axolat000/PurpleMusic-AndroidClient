...
/**
 * Direct client -> Wikipedia lookups for the artist page's bio section. No backend involved: the
 * Wikipedia REST API sends Access-Control-Allow-Origin: *, so it's fetchable straight from any HTTP
 * client. Every call is a fresh live lookup; nothing is cached or persisted.
 */
object WikipediaClient {
    ...
    /**
     * Resolves a bio for [artistName] in [lang], falling back to English if [lang] isn't English and
     * nothing was found there. Blocking network calls inside -- callers must invoke this from a
